...
def user_posts(username):
    ...
    posts_list = Post.query.filter_by(author=user) \
...
def send_reset_email(user):
    ...
    msg.body = f""" To reset your password, please click on below link!
    { url_for('reset_token',token=token, _external=True) }
    If you did not make this request then please ignore the email! No action required.
    """
    mail.send(msg)
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        flash('Reset password email has been sent, please check your inbox!','info')
        return redirect(url_for('login'))
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Password Reset successful! You will now be able to log in', 'success')
        return redirect(url_for('login'))
